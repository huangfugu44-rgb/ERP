  ...
  <div class="sales-return-page">
    <el-card class="search-card">
      ...
      <el-form :inline="true" :model="searchForm">
        ...
        <el-form-item label="原出库单号">
          <el-input v-model="searchForm.outboundNo" placeholder="请输入原出库单号" clearable />
        </el-form-item>
    ...
    <el-card class="table-card">
      <el-table :data="returnList" stripe style="width: 100%">
        ...
        <el-table-column prop="outboundNo" label="原出库单号" width="180">
          <template #default="{ row }">
            <el-button link @click="viewOutbound(row)">{{ row.outboundNo }}</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="formDialogVisible" title="新建退货单" width="900px" destroy-on-close>
      <el-form :model="returnForm" :rules="returnRules" ref="formRef" label-width="100px">
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="原出库单" prop="outboundNo">
              <el-input v-model="returnForm.outboundNo" placeholder="请输入或扫描出库单号">
                <template #append>
                  <el-button @click="loadOutbound">加载</el-button>
                </template>
              </el-input>
            </el-form-item>
          </el-col>
      ...
      <el-table :data="returnForm.items" size="small">
        ...
        <el-table-column prop="outboundQuantity" label="出库数量" width="90" align="center" />
    ...
    <el-dialog v-model="detailDialogVisible" title="退货单详情" width="900px" destroy-on-close>
      <el-descriptions :column="2" border v-if="currentReturn">
        ...
        <el-descriptions-item label="原出库单">{{ currentReturn.outboundNo }}</el-descriptions-item>
...
const returnList = reactive([
  { returnNo: 'SRET20240515001', outboundNo: 'SOUT20240510001', returnDate: '2024-05-15', customerName: '深圳市科技有限公司', warehouseName: '软件仓库', productCount: 1, returnAmount: 5999, refundMethod: '银行转账', status: 'completed', statusText: '已完成', items: [] },
  { returnNo: 'SRET20240510002', outboundNo: 'SOUT20240508002', returnDate: '2024-05-10', customerName: '广州市软件企业', warehouseName: '软件仓库', productCount: 2, returnAmount: 4500, refundMethod: '抵扣货款', status: 'completed', statusText: '已完成', items: [] },
  { returnNo: 'SRET20240505003', outboundNo: 'SOUT20240503003', returnDate: '2024-05-05', customerName: '东莞市网络公司', warehouseName: '软件仓库', productCount: 1, returnAmount: 899, refundMethod: '现金', status: 'cancelled', statusText: '已取消', items: [] }
])
...
const loadOutbound = () => {
  if (!returnForm.outboundNo) {
    ElMessage.warning('请输入出库单号')
    return
  }
  returnForm.customerName = '深圳市科技有限公司'
  returnForm.items = [
    { productName: 'Office 365 商业版', specification: '年订阅', unit: '套', outboundQuantity: 30, returnQuantity: 3, availableReturn: 5, price: 2199 },
    { productName: 'Microsoft 365 E3', specification: '年订阅', unit: '套', outboundQuantity: 20, returnQuantity: 0, availableReturn: 2, price: 1500 }
  ]
  ElMessage.success('已加载出库单商品')
}

const viewOutbound = (row) => ElMessage.info(`查看出库单: ${row.outboundNo}`)
